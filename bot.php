...
// config.php
define('FACEBOOK_PAGE_ACCESS_TOKEN', 'EABC5rFVuO3cBQCEKNKE3ZBY0ZAYY6JDaQJXZAQECpQQZAcIZAp0wmTBFL36ITJ6B2k4ZCLmqyJVtzGtW6mRzzITMZBvJqG7vH6tPO2t2ZCbFdjq0Paic2kngYlsIPoawzRuUOnmPIbZCWycQ0KgDa0ycioFpZBqIZBQuC0droDL8sqfyAiQa4idPBbx76iQ5n1VyU6rZAYaH7cks0gZDZD');
...
function get_sim_card_info($access_token, $msisdn) {
    ...
    return [false, "خطأ في السيرفر: HTTP $http_code"];
}
...
function analyze_sim_card_response($response_data) {
    ...
    try {
        if (isset($response_data['data'])) {
            ...
            $sim_info['msisdn'] = $data['msisdn'] ?? 'غير متوفر';
            $sim_info['imsi'] = $data['imsi'] ?? 'غير متوفر';
            $sim_info['status'] = $data['status'] ?? 'غير متوفر';
            $sim_info['customerType'] = $data['customerType'] ?? 'غير متوفر';
            $sim_info['language'] = $data['language'] ?? 'غير متوفر';
            ...
            if (isset($response_data['subscription-types']) && !empty($response_data['subscription-types'])) {
                ...
                $sim_info['plan_name'] = $subscription['name'] ?? 'غير متوفر';
                $sim_info['plan_type'] = $subscription['type'] ?? 'غير متوفر';
            }
            ...
            if (isset($response_data['connected-products']) && !empty($response_data['connected-products'])) {
                ...
                foreach (array_slice($connected_products, 0, 5) as $product) {
                    $product_info = [
                        ...
                        'status' => $product['status'] ?? 'غير معروف'
                    ];
        ...
        return ["error" => "خطأ في تحليل البيانات: " . $e->getMessage()];
    }
}

function format_sim_card_info($sim_info) {
    if (isset($sim_info['error'])) {
        return "❌ {$sim_info['error']}";
    }
    
    $formatted_message = "📱 **معلومات بطاقة الجيزي**\n\n";
    
    $formatted_message .= "📞 **الرقم:** " . ($sim_info['msisdn'] ?? 'غير متوفر') . "\n";
    $formatted_message .= "🆔 **الحالة:** " . ($sim_info['status'] ?? 'غير متوفر') . "\n";
    $formatted_message .= "👤 **نوع المشترك:** " . ($sim_info['customerType'] ?? 'غير متوفر') . "\n";
    
    if (isset($sim_info['balance'])) {
        $formatted_message .= "💰 **الرصيد:** {$sim_info['balance']} " . ($sim_info['currency'] ?? 'DZD') . "\n";
    } else {
        $formatted_message .= "💰 **الرصيد:** غير متوفر\n";
    }
    
    if (isset($sim_info['plan_name'])) {
        $formatted_message .= "📦 **الباقة:** {$sim_info['plan_name']}\n";
    }
    
    if (isset($sim_info['expiry_date']) && $sim_info['expiry_date'] != 'غير متوفر') {
        $formatted_message .= "📅 **تاريخ الانتهاء:** {$sim_info['expiry_date']}\n";
    }
    
    if (isset($sim_info['last_recharge']) && $sim_info['last_recharge'] != 'غير متوفر') {
        $formatted_message .= "⏰ **آخر شحن:** {$sim_info['last_recharge']}\n";
    }
    
    if (isset($sim_info['activation_date']) && $sim_info['activation_date'] != 'غير متوفر') {
        $formatted_message .= "🕒 **تاريخ التفعيل:** {$sim_info['activation_date']}\n";
    }
    ...
    if (isset($sim_info['active_products']) && !empty($sim_info['active_products'])) {
        ...
        foreach (array_slice($sim_info['active_products'], 0, 3) as $product) {
            $product_text = "• {$product['name']} - {$product['status']}";
            if (isset($product['expiry_date'])) {
                $product_text .= " (ينتهي: {$product['expiry_date']})";
            }
    ...
    if ($user_state["stage"] == "awaiting_msisdn") {
        ...
        if (strlen($msisdn) == 10 && strpos($msisdn, "07") === 0) {
            ...
            if ($user_data) {
                ...
                if ($success) {
                    $display_msisdn = substr($msisdn, 0, 4) . "xxxx" . substr($msisdn, -2);
                    send_facebook_message($sender_id, "📱 تم إرسال رسالة نصية إلى $display_msisdn تحتوي على الرمز.");
        ...
        if (strlen($b_number) == 10 && strpos($b_number, "07") === 0) {
            ...
            if ($success_count > 0) {
                ...
                send_facebook_message($sender_id, "تم ارسال الدعوة بنجاح إلى $display_b_number   الان ماعليك فعله هو فقط ارسال الرقم المدعو الينا والرمز الذي  وصل في رسالة DJEZZY APP او يمكنك الدخول فقط لتطبيق جيزي بالرقم المدعو");
            } else {
...
function verify_webhook() {
    if (isset($_GET['hub_verify_token']) && isset($_GET['hub_challenge'])) {
        $verify_token = 'Nactivi_2025'; // ضع التوكن الخاص بك هنا
